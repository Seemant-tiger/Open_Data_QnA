...
class BQConnector(DBConnector, ABC):
    ...
    def retrieve_matches(self, mode, user_grouping, qe, similarity_threshold, limit, get_only_questions = False):
        """
    ...
    def get_top_match_from_id(
        self, ids, user_question, get_only_questions=False
    ):
        try:
            if ids:
                check_history_sql = f"""SELECT id, example_user_question,example_generated_sql FROM {self.project_id}.{self.opendataqna_dataset}.example_prompt_sql
                                WHERE id in {tuple(ids)};"""

                exact_sql_history = self.client.query_and_wait(
                    check_history_sql
                ).to_dataframe()

                name_txt = ""
                if get_only_questions:
                    for _, r in exact_sql_history.iterrows():
                        example_user_question = r["example_user_question"]
                        example_sql = r["example_generated_sql"]
                        name_txt = name_txt + "\n" + example_user_question
                ...
                for _, r in exact_sql_history.iterrows():
                    ...
                    # if user_question.lower() == example_user_question.lower():
                    #     return example_sql, True
    ...
    def getSimilarMatches(self, mode, user_grouping, qe, num_matches, similarity_threshold, get_only_questions = False ):
        ...
        if mode == 'table':
            ...
            # match_result= self.retrieve_matches(mode, user_grouping, qe, similarity_threshold, num_matches)
            match_result= self.retrieve_matches(mode, user_grouping, qe, similarity_threshold, num_matches, get_only_questions)
    ...
    def getExactMatches(self, query):
        ...
        # check_history_sql=f'''SELECT example_user_question,example_generated_sql FROM {self.project_id}.{self.opendataqna_dataset}.example_prompt_sql_embeddings
        #                   WHERE example_user_question = "{query}" LIMIT 1; '''
        check_history_sql=f'''SELECT example_user_question,example_generated_sql FROM {self.project_id}.{self.opendataqna_dataset}.example_prompt_sql
                          WHERE lower(example_user_question) = lower("{query}") LIMIT 1; '''

        exact_sql_history = self.client.query_and_wait(check_history_sql).to_dataframe()

        print(time.time()-start)
        print(exact_sql_history.to_json())
        start2 = time.time()
